...
<html lang="en">
    <head>
        ...
        <title>Patient Management - HMS Nurse</title>
...
<?= json_encode($patients ?? []) ?>
        ...
        <script>
            (function(){
                const fillSelect = (sel, doctors) => {
                    if (!sel) return;
                    sel.innerHTML = '<option value="">Select doctor...</option>';
                    doctors.forEach(d => {
                        const id = d.doctor_id ?? d.staff_id ?? d.id ?? '';
                        const name = d.full_name || d.name || [d.first_name||'', d.last_name||''].join(' ').trim();
                        if (!id || !name) return;
                ...
                const loadFromNurse = () => fetch('<?= base_url('nurse/doctors/api') ?>', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    .then(r => r.json())
                    .then(res => ({ ok: !!res.success, data: res.data || [] }))
                    .catch(() => ({ ok: false, data: [] }));

                const loadFromDoctor = () => fetch('<?= base_url('doctor/doctors/api') ?>', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    .then(r => r.json())
                    .then(res => ({ ok: (res.status === 'success'), data: res.data || [] }))
                    .catch(() => ({ ok: false, data: [] }));

                (async function(){
                    let resp = await loadFromNurse();
                    if (!resp.ok || resp.data.length === 0) {
                        resp = await loadFromDoctor();
                    }
                    const doctors = resp.data || [];
                    fillSelect(document.getElementById('primary_doctor_id'), doctors);
                    fillSelect(document.getElementById('ep_primary_doctor_id'), doctors);
                })();
            })();
        </script>
        <script src="<?= base_url('js/admin/patient-management.js') ?>"></script>
        <script>
            // Override admin's loadDoctors to prefer nurse API with fallback
            (function(){
                const fillSelect = (sel, doctors, selectedValue) => {
                    if (!sel) return;
                    sel.innerHTML = '';
                    ...
                    (doctors||[]).forEach(d => {
                        ...
                        o.value = id;
                        o.textContent = name;
                        if (selectedValue && String(selectedValue) === String(id)) o.selected = true;
                        sel.appendChild(o);
                    });
                    if (sel.options.length === 1) { // only placeholder
                        sel.options[0].textContent = 'No doctors found';
                    }
                };

                const loadFrom = async (url, successKey) => {
                    try {
                        const r = await fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                        const j = await r.json().catch(()=>({}));
                        if (successKey === 'status') return (j.status === 'success') ? (j.data||[]) : [];
                ...
                window.loadDoctors = async function(selectId, selectedValue){
                    ...
                    let doctors = await loadFrom('<?= base_url('nurse/doctors/api') ?>', 'success');
                    if (!doctors.length) doctors = await loadFrom('<?= base_url('doctor/doctors/api') ?>', 'status');
                    if (!doctors.length) doctors = await loadFrom('<?= base_url('admin/doctors/api') ?>', 'status');
                    fillSelect(sel, doctors, selectedValue);
                };

                // Pre-fill both selects on page load
                window.addEventListener('DOMContentLoaded', function(){
                    if (document.getElementById('primary_doctor_id')) window.loadDoctors('primary_doctor_id');
                    if (document.getElementById('ep_primary_doctor_id')) window.loadDoctors('ep_primary_doctor_id');
                });
            })();
        </script>
